...
- 📝 **メモ表示**: コード名やスケール名などのアノテーションを楽譜に重ねて表示
- 📊 **スコア評価**: 演奏の正確性を評価
- 🔄 **ループ練習**: 楽曲を繰り返し練習
- 🎚️ **BPM調整**: テンポの変更
- 🔊 **音量調整**: 音声フィードバックの調整
...
#### パラメータ説明
- **title**: 楽曲のタイトル（文字列）
- **bpm**: テンポ（60-300、デフォルト: 120）
- **notes**: ノートの配列
  - **pitch**: MIDI音程番号（0-127、60=C4）
  - **beat**: 拍数（0から開始）
  - **duration**: 音符の長さ（拍数、デフォルト: 1）
  - **velocity**: 音の強さ（0-127、デフォルト: 80）
- **memos**: メモ・アノテーションの配列（オプション）
  - **timing.beat**: 表示タイミング（拍数）
  - **text**: 表示するテキスト（例: "C7", "Gペンタトニックスケール"）
  - **align**: テキストの配置（'left' | 'center' | 'right'、デフォルト: 'center'）
  - **color**: カラープリセット（'default' | 'blue' | 'green' | 'purple' | 'orange' | 'pink' | 'red' | 'cyan'、デフォルト: 'default'）
...
### TODO
- [X] URLではなくファイルアップロードによるjsonの読み込み
- [ ] 繰り返し時のスコアの履歴表示
- [X] メモとか小節の区切りとかの表示
- [ ] メトロノーム
- [ ] 音を良くする
